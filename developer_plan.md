...
3. **DEV-001.3 [Tejas Rawool & Samar Shetye]:** Configure GitHub repository access, SSH authentication keys, and create individual working branches (`tejas`, `dakshit`, `nidhish`, `samar`, `shreya`, `safa`) off `main`.
4. **DEV-001.4 [Samar Shetye]:** Configure repository branch protection rules on `main` (require PR review, block force pushes, require passing CI status checks); define Conventional Commit standards.
5. **DEV-001.5 [Tejas Rawool]:** Configure Git pre-commit hooks for formatting (`prettier`, `eslint`, `black`, `flake8`) and secret detection (`detect-secrets`).
6. **DEV-001.6 [Nidhish Dalvi]:** Verify Python runtime configurations (Python 3.11); audit `.gitignore` to guarantee local LLM model weights (`.bin`, `.gguf`, `ollama/`) are strictly excluded.
...
3. **Tejas Rawool (Repository Setup & Client Pre-commit):** Configures repository settings, creates developer working branches, verifies SSH keys for Dakshit and Safa, and sets up Prettier/ESLint pre-commit hooks.
4. **Samar Shetye (Branch Protection & Commit Governance):** Enforces branch protection on `main`, defines Conventional Commit standards (`feat:`, `fix:`, `docs:`, `test:`), and tests policy by verifying direct pushes are blocked.
5. **Nidhish Dalvi (Runtime Validation & Model Weight Exclusion):** Audits Python 3.11 configurations across machines and updates `.gitignore` to prevent multi-gigabyte Ollama model weights from being tracked.
...
### Knowledge Sharing
* **Presenter:** Tejas Rawool & Samar Shetye
...
### Review
Shreya reviews commit logs to ensure no credentials were pushed; Samar Shetye verifies branch protection rules block direct commits to `main`.
...
### Task Sequence
1. **DEV-002.1 [Samar Shetye & Dakshit]:** Standardize Python 3.11 installation across all 6 systems; create isolated virtual environments (`backend/venv`).
2. **DEV-002.2 [Samar Shetye]:** Install backend dependencies (`fastapi`, `uvicorn`, `sqlalchemy`, `pydantic`, `python-docx`, `psutil`, `pytest`) from `backend/requirements.txt`.
3. **DEV-002.3 [Tejas Rawool]:** Standardize Node.js 20 LTS and install frontend packages (`npm install` inside `frontend/`) using Next.js 16 and TailwindCSS.
4. **DEV-002.4 [Nidhish Dalvi]:** Install and start local Ollama engine (`http://localhost:11434`); pull baseline lightweight test model (`qwen2.5:1.5b` or `qwen2.5:7b`).
...
### Developer Responsibilities (In Execution Order)
1. **Samar Shetye (Backend Environment Lead):** Starts by establishing the clean Python 3.11 virtual environment and installing backend packages, verifying zero version conflicts between SQLAlchemy and Pydantic v2.
...
3. **Tejas Rawool (Frontend Tooling Lead):** Guides Node.js 20 LTS standardization and executes `npm install` across all machines, verifying that Next.js 16 compiles without warnings.
4. **Nidhish Dalvi (Local AI Engine Lead):** Leads the Ollama installation session; tests loopback connectivity (`curl http://localhost:11434/api/tags`); verifies model pulling script `scripts/pull_models.sh`.
...
### Collaboration Flow
Samar Shetye sets up the Python foundation; Tejas Rawool brings up the Node.js frontend packages; Nidhish Dalvi screen-shares Ollama setup and model pulling; Shreya audits configuration security; Dakshit and Safa validate the setup across all machines.
...
### Review
Samar Shetye checks that all 6 backend virtual environments activate cleanly; Tejas Rawool inspects that `npm run build` succeeds without package errors.
...
### Task Sequence
1. **DEV-003.1 [Samar Shetye]:** Boot FastAPI backend (`uvicorn app.main:app --reload --port 8000`) and verify `/health` endpoint returns `200 OK`.
2. **DEV-003.2 [Tejas Rawool]:** Boot Next.js frontend (`npm run dev -- -p 3000`) and verify browser opens the Sovereign AI Workbench console at `http://localhost:3000`.
3. **DEV-003.3 [Nidhish Dalvi]:** Test backend LLM fallback execution in `app/api/chat.py`; verify how prompt classification interacts with Ollama.
...
### Developer Responsibilities (In Execution Order)
1. **Samar Shetye (Backend Server Dispatcher):** Starts the FastAPI application gateway on port 8000, demonstrates the OpenAPI `/docs` interface, and walks through the SSE event broadcaster and database models.
2. **Tejas Rawool (Frontend Console Dispatcher):** Boots the Next.js frontend on port 3000, demonstrates the 3-column AppShell, and triggers the first live REST call to the backend.
3. **Nidhish Dalvi (LLM Loopback Tester):** Submits test engineering prompts to `POST /api/chat`, observes Ollama prompt processing in terminal logs, and verifies response generation.
...
### Collaboration Flow
Samar Shetye starts the backend; Tejas Rawool starts the frontend; Nidhish Dalvi sends the first test prompt; Dakshit traces the UI state; Shreya verifies network confinement; Safa records gaps and consolidates the sprint log.
...
### Knowledge Sharing
* **Presenter:** Samar Shetye & Tejas Rawool
...
### Review
Cross-verification: Tejas Rawool verifies Dakshit's and Safa's UI execution; Samar Shetye verifies Shreya's and Nidhish Dalvi's backend execution.
...
### Task Sequence
1. **DEV-004.1 [Samar Shetye]:** Conduct architecture review session covering all 9 layers (UI $\to$ Gateway $\to$ Sentinel $\to$ Router $\to$ Policy $\to$ LangGraph $\to$ Tools/RAG $\to$ Critique $\to$ Human Sign-off).
...
3. **DEV-004.3 [Nidhish Dalvi]:** Define routing and LLM state fields (`selected_model`, `routing_rationale`, `llm_raw_response`, `reasoning_tokens`); map prompt schema inputs.
...
5. **DEV-004.5 [Tejas Rawool]:** Map frontend rendering requirements into state fields (node step states, router confidence cards, citation card excerpts).
6. **DEV-004.6 [Samar Shetye]:** Lead the construction of `backend/app/schemas/state.py`; implement Pydantic v2 `BaseModel` with validation rules; verify JSON serialization.
...
### Developer Responsibilities (In Execution Order)
1. **Samar Shetye (System Architecture Lead):** Walks the team through the 9-layer data lifecycle of an NDT report and scaffolds `backend/app/schemas/state.py`.
...
3. **Nidhish Dalvi (LLM & Router State Architect):** Supplies state fields for model selection, routing justification, and reasoning tokens.
...
5. **Tejas Rawool (Frontend State Alignment):** Reviews state fields against UI display components, ensuring all telemetry items can be extracted cleanly.
...
### Collaboration Flow
Samar Shetye leads the architecture review and screen-shares the code. Dakshit, Nidhish Dalvi, and Safa provide specialized field requirements. Tejas Rawool ensures client compatibility. Shreya signs off on security and immutability.
...
### Review
Nidhish Dalvi reviews state fields for LLM compatibility; Tejas Rawool reviews state fields for TypeScript interface alignment.
...
### Task Sequence
1. **DEV-005.1 [Samar Shetye]:** Finalize REST API endpoints: `POST /api/chat`, `POST /api/upload`, `GET /api/tasks/{id}`, `POST /api/tasks/{id}/approve`, `GET /api/tasks/{id}/download`, `GET /api/network/status`.
2. **DEV-005.2 [Samar Shetye]:** Implement standardized Pydantic request and response schemas in `backend/app/schemas/api.py`.
3. **DEV-005.3 [Tejas Rawool]:** Create mirroring TypeScript interfaces in `frontend/src/types/api.ts` and `frontend/src/types/task.ts`; update `frontend/src/lib/api.ts`.
...
5. **DEV-005.5 [Nidhish Dalvi]:** Define `/api/chat` payload parameters (temperature, model preference, system prompt overrides); ensure fallback responses follow structured error schemas.
...
### Developer Responsibilities (In Execution Order)
1. **Samar Shetye (API Gateway Architect):** Authors the core route definitions and Pydantic schemas in `backend/app/api/` and exports the generated OpenAPI specification (`openapi.json`).
2. **Tejas Rawool (TypeScript Client Architect):** Consumes the OpenAPI schema, builds typed API methods in `frontend/src/lib/api.ts`, and establishes typed SSE event listeners.
...
4. **Nidhish Dalvi (Chat Payload Specialist):** Validates request parameters for local model inference and structures standardized error payloads.
...
### Collaboration Flow
Samar Shetye establishes the server endpoints; Tejas Rawool immediately mirrors them into client TypeScript interfaces; Dakshit and Nidhish Dalvi refine upload and chat parameters; Shreya secures headers; Safa executes the contract test suite.
...
### Knowledge Sharing
* **Presenter:** Tejas Rawool & Dakshit
...
### Review
Dakshit reviews backend schemas; Nidhish Dalvi reviews frontend TypeScript interfaces; Samar Shetye reviews error handling logic.
...
### Task Sequence
1. **DEV-006.1 [Samar Shetye]:** Refactor `backend/app/core/sse_manager.py` to support multi-client subscription queues and connection heartbeat pings.
2. **DEV-006.2 [Samar Shetye & Nidhish Dalvi]:** Create an integration test harness (`scripts/simulate_agent_flow.py`) emitting realistic sequential agent steps (`Ingest` $\to$ `Route` $\to$ `OCR` $\to$ `RAG` $\to$ `Sandbox` $\to$ `Critique` $\to$ `HITL`).
3. **DEV-006.3 [Tejas Rawool]:** Wire frontend `ContextPanel.tsx` and `ExecutionTimeline.tsx` to display real-time node state transitions (active, completed, error).
...
### Developer Responsibilities (In Execution Order)
1. **Samar Shetye (SSE Event Broadcaster):** Implements the pub-sub connection queue in `sse_manager.py` with 15-second heartbeat pings.
2. **Nidhish Dalvi (Trace Payload Synthesizer):** Generates realistic simulated agent step payloads (thickness numbers, API clauses, severity ratings) for the simulation script.
3. **Tejas Rawool (UI Stream Visualizer):** Connects the frontend `ExecutionTimeline` to the live SSE stream, animating node transitions as events arrive.
...
### Collaboration Flow
Samar Shetye and Nidhish Dalvi launch the backend trace generator; Tejas Rawool connects the frontend UI; Dakshit tests client resilience; Shreya audits server sockets; Safa performs cross-laptop verification.
...
### Review
Nidhish Dalvi and Dakshit review frontend state handling; Shreya and Safa review backend SSE connection cleanup.
...
### Task Sequence
1. **DEV-007.1 [Samar Shetye]:** Install PostgreSQL 16 locally (or launch official PostgreSQL container on port 5432); create database `abhedya_db` and role `abhedya_user`.
2. **DEV-007.2 [Samar Shetye & Dakshit]:** Design complete relational schema DDL: `tasks`, `agent_steps`, `documents`, `approvals`, `deliverables`, and `audit_logs`.
3. **DEV-007.3 [Samar Shetye]:** Initialize Alembic migration environment (`backend/alembic/`) and configure `env.py` to bind with SQLAlchemy ORM models.
...
5. **DEV-007.5 [Tejas Rawool & Nidhish Dalvi]:** Review schema relationships to ensure task history, document listings, and step outputs can be queried efficiently with pagination.
...
### Developer Responsibilities (In Execution Order)
1. **Samar Shetye (Database Architect):** Configures the PostgreSQL instance, writes the SQLAlchemy ORM models in `backend/app/db/models.py`, and initializes Alembic.
...
4. **Nidhish Dalvi (JSONB Schema Reviewer):** Validates that `agent_steps.input_data` and `output_data` JSONB columns accept arbitrary model reasoning outputs without schema alterations.
5. **Tejas Rawool (Query Performance Reviewer):** Ensures appropriate database indexes exist on `task_id` and `created_at` to support fast UI pagination.
...
### Collaboration Flow
Samar Shetye and Dakshit build the SQL foundation; Shreya secures connection profiles; Nidhish Dalvi and Tejas Rawool verify querying needs; Safa executes and tests migration rollbacks.
...
### Review
Dakshit verifies table structures; Nidhish Dalvi tests JSONB column queries; Samar Shetye validates foreign key cascade constraints.
...
### Task Sequence
1. **DEV-008.1 [Samar Shetye]:** Implement database session context manager (`get_db`) ensuring automatic commit on success and rollback on exception.
2. **DEV-008.2 [Samar Shetye]:** Build `TaskRepository` methods (`create_task`, `update_task_status`, `get_by_id`, `list_tasks`) with strict status transition checks.
...
4. **DEV-008.4 [Nidhish Dalvi]:** Implement `StepRepository` methods (`create_step`, `get_steps_for_task`); verify JSONB serialization for complex LLM outputs.
...
7. **DEV-008.7 [Tejas Rawool]:** Test repository methods against frontend queries; verify that `GET /api/tasks` returns paginated task history with correct timestamps.

### Developer Responsibilities (In Execution Order)
1. **Samar Shetye (Repository Pattern Lead):** Builds the core transaction manager and `TaskRepository`, enforcing legal status transitions (`pending` $\to$ `processing` $\to$ `completed`).
...
3. **Nidhish Dalvi (Agent Step Persistence Specialist):** Builds `StepRepository` handling JSONB serialization of model thoughts, tool inputs, and outputs.
...
6. **Tejas Rawool (Client Query Integrator):** Connects repository outputs to API route handlers, verifying paginated task history displays properly in UI.

### Collaboration Flow
Samar Shetye, Dakshit, and Nidhish Dalvi write repository classes; Safa writes the approval persistence; Shreya executes rollback stress testing; Tejas Rawool verifies API responses.
...
### Knowledge Sharing
* **Presenter:** Dakshit & Samar Shetye
* **Topic:** "The Repository Pattern in FastAPI: Decoupling SQL Operations from Business Logic for High Reliability."
* **Duration:** 30 minutes during Saturday online session.

### Review
Samar Shetye reviews repository code for SQL injection vulnerabilities; Shreya reviews session scoping to prevent connection pool exhaustion.
...
2. **DEV-009.2 [Samar Shetye]:** Create database triggers/listeners appending audit events on every state transition, file upload, and human approval.
...
4. **DEV-009.4 [Nidhish Dalvi]:** Test audit event generation for model routing decisions; ensure model name, prompt tokens, and rationale are immutably logged.
...
7. **DEV-009.7 [Tejas Rawool]:** Build frontend `AuditTrailView.tsx` displaying the immutable hash chain, sequence numbers, event types, and a visual "Chain Verified" badge.
...
2. **Samar Shetye (Database Event Hook Engineer):** Hooks the audit logger into the database repository layer, ensuring every task transition automatically appends a block.
...
4. **Nidhish Dalvi (AI Decision Audit Specialist):** Formats model routing and prompt audit payloads, ensuring AI reasoning is recorded for compliance.
...
6. **Tejas Rawool (Audit Interface Engineer):** Builds the `AuditTrailView` in the frontend, rendering the sequence of blocks with real-time verification indicators.

### Collaboration Flow
Shreya establishes the cryptographic core; Samar Shetye wires it to database transactions; Dakshit and Nidhish Dalvi supply event payloads; Safa tests tampering detection; Tejas Rawool visualizes the ledger in the UI.
...
### Review
Samar Shetye reviews hash calculation performance; Tejas Rawool reviews audit UI rendering; Nidhish Dalvi verifies payload determinism.
...
### Task Sequence
1. **DEV-010.1 [Nidhish Dalvi]:** Standardize local model portfolio: `qwen2.5:14b` (reasoning/standards), `deepseek-r1:14b` or `deepseek-r1:8b` (math/code), and `qwen2.5-vl:7b` (vision).
2. **DEV-010.2 [Nidhish Dalvi]:** Write automated model verification script `scripts/pull_models.sh` that checks available local models and pulls missing weights.
3. **DEV-010.3 [Samar Shetye]:** Implement asynchronous Ollama client wrapper in `backend/app/core/llm.py` with keep-alive management and connection pooling.
...
6. **DEV-010.6 [Tejas Rawool]:** Test Ollama streaming API; ensure chunk-by-chunk token emission can be streamed over SSE to the frontend with low latency.
...
### Developer Responsibilities (In Execution Order)
1. **Nidhish Dalvi (Model Portfolio Lead):** Selects optimal model quantizations (Q4_K_M), creates `scripts/pull_models.sh`, and implements `<think>...</think>` tag stripping.
2. **Samar Shetye (Async LLM Client Architect):** Writes the asynchronous client in `backend/app/core/llm.py`, handling connection drops and timeouts gracefully.
...
5. **Tejas Rawool (Token Stream Integrator):** Connects the backend token yield generator to the frontend message stream, measuring rendering latency.
...
### Collaboration Flow
Nidhish Dalvi and Samar Shetye configure the runtime and async client; Dakshit tests vision models; Shreya validates network security; Tejas Rawool tests streaming; Safa compiles performance benchmarks.
...
### Review
Samar Shetye reviews async client connection management; Shreya verifies loopback network binding.
...
### Task Sequence
1. **DEV-011.1 [Nidhish Dalvi]:** Build query classification heuristic in `backend/app/agent/router.py` (categorizing tasks into: `VISUAL_INSPECTION`, `CORROSION_MATH`, `SOP_RETRIEVAL`, `GENERAL_QUERY`).
2. **DEV-011.2 [Nidhish Dalvi]:** Implement multi-factor routing scoring function:
   $$\text{Score}(M, T) = w_1 \cdot \text{ModalityMatch} + w_2 \cdot \text{ComplexityFit} + w_3 \cdot \text{VRAMHeadroom} - w_4 \cdot \text{Latency}$$
3. **DEV-011.3 [Samar Shetye]:** Integrate router into task pipeline; verify router execution completes in $<50\text{ ms}$ without blocking the async event loop.
...
7. **DEV-011.7 [Tejas Rawool]:** Update `ModelRouterCard.tsx` in frontend to render the router's explainable rationale, VRAM meter, and selected model badge.

### Developer Responsibilities (In Execution Order)
1. **Nidhish Dalvi (Routing Algorithm Architect):** Implements the deterministic heuristic classifier and multi-factor scoring algorithm in `backend/app/agent/router.py`.
2. **Samar Shetye (Pipeline Integration Lead):** Connects the router output into the task pipeline, ensuring structured decisions are emitted over SSE.
...
6. **Tejas Rawool (Router UI Visualizer):** Updates the frontend `ModelRouterCard` to present the selection rationale clearly to the operator.

### Collaboration Flow
Nidhish Dalvi writes the core heuristic; Samar Shetye integrates it into the pipeline; Dakshit and Safa feed domain test prompts; Shreya tests VRAM exhaustion edge cases; Tejas Rawool visualizes the decision in the UI.
...
### Knowledge Sharing
* **Presenter:** Nidhish Dalvi & Dakshit
...
### Review
Samar Shetye reviews router latency and algorithmic complexity; Shreya verifies safe degradation when VRAM is saturated.
...
### Task Sequence
1. **DEV-012.1 [Nidhish Dalvi & Shreya]:** Implement L0–L4 policy engine in `backend/app/core/policy.py`:
...
3. **DEV-012.3 [Nidhish Dalvi]:** Implement prompt injection filter scanning incoming prompts for jailbreaks, prompt leaks, and role overrides.
4. **DEV-012.4 [Samar Shetye]:** Implement structured JSON extraction utility (`extract_json_from_text`) handling markdown fences, malformed braces, and trailing commas.
...
6. **DEV-012.6 [Tejas Rawool]:** Implement policy warning banners in frontend `ChatContainer.tsx` displaying policy violation notices when prompts are blocked.

### Developer Responsibilities (In Execution Order)
1. **Nidhish Dalvi (Constitutional Engine Lead):** Authors the system prompt wrappers and L0–L4 policy rules in `backend/app/core/policy.py`.
...
4. **Samar Shetye (JSON Parser Engineer):** Writes the resilient JSON extraction parser, recovering structured outputs from noisy LLM responses.
...
6. **Tejas Rawool (Policy Alert UI Engineer):** Builds alert modals and warning banners in the frontend when an operator submits a blocked prompt.

### Collaboration Flow
Nidhish Dalvi and Shreya build the policy rules; Dakshit stress tests with adversarial attacks; Samar Shetye implements the JSON parser; Safa verifies audit logging; Tejas Rawool builds UI alerts.
...
### Task Sequence
1. **DEV-013.1 [Samar Shetye]:** Implement secure file ingestion endpoint `POST /api/upload` storing files in segregated directory `/data/uploads/` with UUID prefixes.
...
4. **DEV-013.4 [Nidhish Dalvi]:** Test extracted text feeds into LLM prompt contexts; verify truncation safety for documents exceeding token context limits.
5. **DEV-013.5 [Tejas Rawool]:** Build file upload dropzone in frontend `Composer.tsx` with drag-and-drop, upload progress bar, and file type validation.
...
### Developer Responsibilities (In Execution Order)
1. **Samar Shetye (Upload Gateway Engineer):** Implements `POST /api/upload`, filesystem storage paths, and metadata persistence in PostgreSQL.
...
4. **Nidhish Dalvi (Context Budget Specialist):** Verifies that extracted document text is cleanly segmented and fits within the model's token context window.
5. **Tejas Rawool (Upload Interface Lead):** Builds the drag-and-drop file upload zone in `Composer.tsx` with live progress bars and file previews.
...
### Collaboration Flow
Samar Shetye and Shreya secure the upload endpoint; Dakshit implements format extractors; Nidhish Dalvi tests context chunking; Tejas Rawool builds the UI dropzone; Safa verifies the real test files.
...
### Review
Shreya reviews file validation security; Samar Shetye reviews database record creation; Tejas Rawool tests upload UI error handling.
...
3. **DEV-014.3 [Samar Shetye]:** Implement async thread-pool worker for OCR processing to prevent CPU-intensive computer vision tasks from freezing the FastAPI event loop.
4. **DEV-014.4 [Nidhish Dalvi]:** Connect OCR output to the prompt context; verify that structured tables format cleanly as Markdown tables for LLM reasoning.
...
6. **DEV-014.6 [Tejas Rawool]:** Build tabular data preview modal in frontend allowing operators to inspect and verify OCR-extracted tables before agent processing.
...
2. **Samar Shetye (Async Execution Specialist):** Wraps OCR processing in an async background executor, ensuring the server handles concurrent requests without freezing.
3. **Nidhish Dalvi (Context Formatter):** Converts raw OCR bounding-box dictionaries into clean markdown tables formatted for LLM reasoning.
...
5. **Tejas Rawool (Table Preview UI Engineer):** Builds the interactive modal allowing operators to view and edit OCR-extracted tables before submission.
6. **Safa (OCR Accuracy Evaluator):** Benchmarks OCR accuracy against 5 real-world scanned sheets, measuring numerical precision.

### Collaboration Flow
Dakshit leads PaddleOCR integration; Samar Shetye manages thread-pool concurrency; Nidhish Dalvi formats the tables; Shreya verifies offline execution; Tejas Rawool builds the preview UI; Safa evaluates accuracy.
...
### Review
Dakshit and Samar Shetye review OCR performance and memory utilization; Shreya verifies local-only model weight loading.
...
### Task Sequence
1. **DEV-015.1 [Nidhish Dalvi]:** Configure `qwen2.5-vl:7b` in Ollama; build multimodal query helper in `backend/app/core/llm.py` supporting base64-encoded image payloads.
2. **DEV-015.2 [Dakshit & Nidhish Dalvi]:** Build visual defect inspection tool (`backend/app/tools/vision_tool.py`) detecting pitting corrosion, flange leaks, and weld defects from photographs.
3. **DEV-015.3 [Samar Shetye]:** Manage memory during dual OCR and VLM execution; ensure image buffers are cleared from RAM immediately following inference.
...
5. **DEV-015.5 [Tejas Rawool]:** Update frontend chat message renderer (`MessageBubble.tsx`) to display uploaded images alongside visual inspection findings and bounding boxes.
...
### Developer Responsibilities (In Execution Order)
1. **Nidhish Dalvi (VLM Integration Lead):** Builds base64 image encoding and multi-modal request formatting for Ollama's API.
...
3. **Samar Shetye (Memory & Buffer Manager):** Implements immediate memory cleanup for high-resolution image tensors post-inference.
4. **Shreya (Image Asset Security Auditor):** Audits image storage directories, verifying permissions and path isolation.
5. **Tejas Rawool (Visual Telemetry UI Lead):** Implements image rendering, zoom modals, and bounding-box overlays in the chat interface.
...
### Collaboration Flow
Nidhish Dalvi and Dakshit code the VLM integration; Samar Shetye manages memory buffers; Shreya audits file access; Tejas Rawool renders visual outputs in the UI; Safa validates defect detection.
...
### Review
Samar Shetye reviews memory cleanup; Tejas Rawool reviews image rendering in UI; Shreya audits file access permissions.
...
### Task Sequence
1. **DEV-016.1 [Samar Shetye]:** Deploy Qdrant vector database service locally (native binary or Docker container) exposing port 6333 with persistent local storage.
...
3. **DEV-016.3 [Samar Shetye & Dakshit]:** Create initialized Qdrant collection `sovereign_sops` with cosine distance metric and HNSW indexing parameters.
...
5. **DEV-016.5 [Tejas Rawool]:** Add Qdrant service status indicator to frontend `Header.tsx` and `ContextPanel.tsx` reflecting vector database connectivity.
...
### Developer Responsibilities (In Execution Order)
1. **Samar Shetye (Vector Engine Deployer):** Starts the Qdrant service on port 6333, configures persistent volume mounts, and builds the connection client.
...
4. **Nidhish Dalvi (Chunking Strategy Specialist):** Formulates chunking boundaries (512 tokens) preserving regulatory section numbers and headers.
5. **Tejas Rawool (Service Telemetry Integrator):** Adds Qdrant health indicators and collection status meters to the frontend header.
...
### Collaboration Flow
Samar Shetye and Dakshit deploy Qdrant and embeddings; Shreya validates offline isolation; Nidhish Dalvi optimizes chunking; Tejas Rawool updates UI telemetry; Safa verifies multi-machine connectivity.
...
### Knowledge Sharing
* **Presenter:** Samar Shetye & Dakshit
...
### Review
Shreya audits offline model caching; Samar Shetye reviews connection pooling and error recovery.
...
### Task Sequence
1. **DEV-017.1 [Nidhish Dalvi]:** Curate and format synthetic industrial SOP corpus in `data/sample_sops/` (API 570, API 510, API 653, OISD-105).
...
3. **DEV-017.3 [Samar Shetye]:** Implement batch ingestion CLI pipeline reading SOP files, generating embeddings, and uploading points to Qdrant with retry logic.
...
5. **DEV-017.5 [Tejas Rawool]:** Build `#Collection` selector dropdown in frontend `Composer.tsx` allowing operators to scope retrieval to specific units (e.g. `#Hydrocracker`).
6. **DEV-017.6 [Safa]:** Execute the ingestion script; verify vector point counts in Qdrant web dashboard; validate metadata tags.

### Developer Responsibilities (In Execution Order)
1. **Nidhish Dalvi (SOP Corpus Curator):** Structures realistic engineering standards with precise technical clauses, tables, and formula definitions.
...
3. **Samar Shetye (Batch Indexing Pipeline Lead):** Builds the high-throughput batch indexer, writing points to Qdrant with metadata payloads.
4. **Shreya (Data Sanitation Auditor):** Reviews indexed payloads to ensure clean regulatory text with zero proprietary secrets.
5. **Tejas Rawool (Collection Selector UI Lead):** Adds the `#Collection` tagging filter to the prompt composer in the frontend.
...
### Collaboration Flow
Nidhish Dalvi curates documents; Dakshit and Samar Shetye build the batch indexing engine; Shreya audits data; Tejas Rawool builds collection selectors; Safa verifies indexing.
...
### Knowledge Sharing
* **Presenter:** Dakshit & Nidhish Dalvi
...
### Review
Samar Shetye reviews indexing batch throughput; Nidhish Dalvi reviews chunk boundary coherence; Shreya audits metadata integrity.
...
2. **DEV-018.2 [Samar Shetye]:** Benchmark retriever latency; ensure search, filtering, and formatting complete in $<150\text{ ms}$.
3. **DEV-018.3 [Nidhish Dalvi]:** Integrate retrieved chunks into the LLM context prompt; implement prompt delimiters (`<context>...</context>`) preventing hallucinated citations.
...
5. **DEV-018.5 [Tejas Rawool]:** Build and polish `RAGSourceCard.tsx` in frontend with expandable excerpt view, similarity score badge, and source page link.
...
2. **Samar Shetye (Query Optimizer):** Optimizes Qdrant client connection pooling, keeping retrieval times strictly under 150ms.
3. **Nidhish Dalvi (Context Prompt Assembler):** Formats retrieved clauses into the model prompt, instructing the LLM to cite specific clauses.
4. **Shreya (Access Isolation Auditor):** Verifies that scoped queries cannot access documents from unselected plant collections.
5. **Tejas Rawool (Citation Card UI Lead):** Renders interactive citation cards in the chat feed with direct page links and text snippets.
...
### Collaboration Flow
Dakshit and Samar Shetye build the retrieval logic; Nidhish Dalvi formats the context; Shreya verifies isolation; Tejas Rawool renders citation cards; Safa measures accuracy.
...
### Knowledge Sharing
* **Presenter:** Dakshit & Tejas Rawool
* **Topic:** "Grounding AI in Authoritative Engineering Truth: Vector Similarity, Threshold Gating & Interactive UI Citations."
* **Duration:** 35 minutes during Sunday online session.

### Review
Nidhish Dalvi reviews citation fidelity; Samar Shetye reviews async query efficiency; Shreya verifies tag-based isolation.
...
### Task Sequence
1. **DEV-019.1 [Samar Shetye]:** Install `langgraph` in the backend environment; create module `backend/app/agent/`.
2. **DEV-019.2 [Samar Shetye]:** Define state graph topology in `backend/app/agent/graph.py` with nodes:
   `ingest` $\to$ `route` $\to$ `extract` $\to$ `retrieve` $\to$ `plan` $\to$ `sandbox` $\to$ `critique` $\to$ `human_checkpoint` $\to$ `synthesize`.
3. **DEV-019.3 [Nidhish Dalvi]:** Define routing node edge conditions; ensure router output directs execution to the appropriate specialized sub-graphs.
...
6. **DEV-019.6 [Tejas Rawool]:** Align frontend `ExecutionTimeline.tsx` with exact LangGraph node names to ensure 1-to-1 visual synchronization of active steps.
...
### Developer Responsibilities (In Execution Order)
1. **Samar Shetye (LangGraph Architect):** Builds the core `StateGraph(WorkbenchState)` and compiles the graph topology in `backend/app/agent/graph.py`.
2. **Nidhish Dalvi (Conditional Edge Designer):** Configures routing conditions and critique evaluation branch decisions.
...
5. **Tejas Rawool (UI Stepper Synchronizer):** Updates frontend step definitions in `ExecutionTimeline.tsx` to match graph node names exactly.
6. **Safa (State Machine Visualizer & Scribe):** Generates and publishes the official Mermaid state diagram and documentation.

### Collaboration Flow
Samar Shetye builds the state graph; Nidhish Dalvi sets up edge branching; Shreya adds loop guards; Dakshit checks data flow; Tejas Rawool aligns the UI timeline; Safa produces the diagram.
...
### Knowledge Sharing
* **Presenter:** Samar Shetye
* **Topic:** "Cyclic State Machines vs Linear Chains: Why LangGraph Solves the Fragility of Traditional LLM Pipelines."
* **Duration:** 25 minutes during Friday offline sync.

### Review
Shreya verifies infinite-loop guards; Samar Shetye reviews state persistence across nodes; Tejas Rawool confirms node names match frontend timeline.
...
2. **DEV-020.2 [Samar Shetye]:** Implement `route_node`: invokes the Adaptive Model Router and records explainable model selection in state.
...
4. **DEV-020.4 [Nidhish Dalvi]:** Implement `plan_node`: formats engineering context and prompts the selected LLM to formulate an inspection analysis plan.
...
6. **DEV-020.6 [Tejas Rawool]:** Verify that each node execution dispatches an SSE event (`emit_step`) displaying node progress and intermediate output in the UI.
...
2. **Samar Shetye (Route Node Engineer):** Writes `route_node`, binding the Adaptive Model Router to the graph state.
3. **Nidhish Dalvi (Plan Node Engineer):** Writes `plan_node`, assembling prompts for LLM reasoning and generating structured engineering plans.
...
5. **Tejas Rawool (Step Emission Integrator):** Connects node transitions to live SSE broadcasting, displaying step cards in the UI.
6. **Safa (Node Unit Test Lead):** Authors `backend/tests/test_agent_nodes.py` and verifies all nodes execute cleanly in isolation.

### Collaboration Flow
Dakshit, Samar Shetye, and Nidhish Dalvi implement specialized nodes; Shreya reviews data security; Tejas Rawool verifies step event emission; Safa leads unit testing.
...
### Knowledge Sharing
* **Presenter:** Samar Shetye & Nidhish Dalvi
* **Topic:** "Designing Isolated Agent Nodes: Functional State Transformation, Pure Functions & State Immutability."
* **Duration:** 30 minutes during Saturday online session.

### Review
Dakshit reviews retrieval node queries; Nidhish Dalvi reviews prompt assembly; Samar Shetye validates state mutation rules.
...
### Task Sequence
1. **DEV-021.1 [Samar Shetye]:** Refactor `backend/app/api/chat.py` to initialize and invoke the compiled LangGraph graph using `graph.astream()`.
2. **DEV-021.2 [Samar Shetye]:** Implement background task dispatch: return `200 OK` with `task_id` immediately to client while graph executes asynchronously.
3. **DEV-021.3 [Tejas Rawool]:** Test frontend chat stream handling; verify that real-time tokens and step transitions update live in the UI without browser stutter.
...
5. **DEV-021.5 [Nidhish Dalvi]:** Monitor Ollama token generation during graph execution; ensure long inferences do not trigger Uvicorn worker timeouts.
...
### Developer Responsibilities (In Execution Order)
1. **Samar Shetye (Async Execution Architect):** Replaces the legacy procedural chat runner with `graph.astream()`, broadcasting step events to the SSE queue.
2. **Tejas Rawool (Stream Consumer Integrator):** Verifies that the frontend receives and displays real-time state transitions without UI freezing.
...
4. **Nidhish Dalvi (Inference Timeout Specialist):** Tunes Ollama timeout parameters to ensure complex multi-node reasoning completes safely.
...
### Collaboration Flow
Samar Shetye leads backend refactoring; Tejas Rawool synchronizes client stream handling; Dakshit and Nidhish Dalvi verify context and LLM inference; Shreya monitors server health; Safa executes concurrency tests.
...
### Review
Nidhish Dalvi reviews prompt response quality; Samar Shetye reviews async task lifecycle; Shreya audits database connection pool under load.
...
### Task Sequence
1. **DEV-022.1 [Shreya & Samar Shetye]:** Build lightweight sandbox container image (`docker/sandbox.Dockerfile`) containing Python 3.11, `numpy`, `scipy`, and `openpyxl`.
...
3. **DEV-022.3 [Samar Shetye]:** Implement sandbox execution runner in `backend/app/tools/sandbox.py` using Docker SDK for Python with `--network none` and 512MB RAM cap.
...
5. **DEV-022.5 [Nidhish Dalvi]:** Craft code generation prompt instructing LLM to generate pure Python calculation functions without external network dependencies.
6. **DEV-022.6 [Tejas Rawool]:** Build code execution card in frontend `MessageBubble.tsx` displaying the generated Python script, execution output, and sandbox status badge.
...
2. **Samar Shetye (Docker Runner Engineer):** Implements container lifecycle management in `backend/app/tools/sandbox.py`, ensuring containers are destroyed immediately.
3. **Dakshit (Engineering Math Specialist):** Writes the calculation formulas for corrosion rate ($CR$) and remaining life ($RL$).
4. **Nidhish Dalvi (Code Prompt Engineer):** Formulates code generation prompts enforcing standard Python math syntax without dangerous imports.
5. **Tejas Rawool (Code Execution UI Lead):** Renders the generated script and execution results in a collapsible code block in the chat UI.
...
### Collaboration Flow
Shreya and Samar Shetye construct the hardened container runner; Dakshit and Nidhish Dalvi provide engineering calculations and prompts; Tejas Rawool builds the UI display; Safa executes security boundary testing.
...
### Knowledge Sharing
* **Presenter:** Shreya & Samar Shetye
* **Topic:** "Hardened Sandboxes for Untrusted LLM Code: Docker Network Denial, cgroups Resource Quotas & AST Pre-Validation."
* **Duration:** 25 minutes during Friday offline sync.

### Review
Shreya audits container isolation flags; Samar Shetye reviews container cleanup; Nidhish Dalvi verifies math accuracy.
...
### Task Sequence
1. **DEV-023.1 [Nidhish Dalvi]:** Implement critique evaluation node in `backend/app/agent/nodes/self_rag.py`.
2. **DEV-023.2 [Nidhish Dalvi]:** Implement **`ISREL`** metric: evaluates whether retrieved SOP clauses are semantically relevant to the user query ($\text{Threshold} \ge 0.75$).
3. **DEV-023.3 [Nidhish Dalvi]:** Implement **`ISSUP`** metric: evaluates whether every factual claim in the generated recommendation is supported by retrieved text ($\text{Threshold} \ge 0.80$).
4. **DEV-023.4 [Samar Shetye]:** Wire critique node into LangGraph state machine; manage conditional edge branching and revision counter limits (`max_retries=2`).
...
7. **DEV-023.7 [Tejas Rawool]:** Build visual verification badge in frontend showing `ISREL` and `ISSUP` scores, factual grounding status, and revision count.

### Developer Responsibilities (In Execution Order)
1. **Nidhish Dalvi (Self-RAG Algorithm Lead):** Implements the `ISREL` and `ISSUP` evaluation prompts and scoring functions in `self_rag.py`.
2. **Samar Shetye (Cyclic Revision Edge Engineer):** Wires the conditional branch in LangGraph, routing failed evaluations back to the planning node.
...
6. **Tejas Rawool (Critique Badge UI Lead):** Renders the `ISREL`/`ISSUP` verification scores and revision badges in the frontend chat bubble.

### Collaboration Flow
Nidhish Dalvi codes the critique metrics; Samar Shetye connects the revision loop; Shreya and Dakshit audit safety thresholds; Safa feeds synthetic hallucination tests; Tejas Rawool visualizes scores in the UI.
...
### Knowledge Sharing
* **Presenter:** Nidhish Dalvi & Safa
* **Topic:** "Automated Hallucination Prevention: Implementing Self-RAG Critique Gates (ISREL & ISSUP) in Safety-Critical AI."
* **Duration:** 30 minutes during Saturday online session.

### Review
Samar Shetye reviews graph loop termination; Nidhish Dalvi reviews evaluation prompt consistency; Shreya audits threshold strictness.
...
### Task Sequence
1. **DEV-024.1 [Samar Shetye]:** Configure LangGraph checkpointing with PostgreSQL checkpointer (`AsyncPostgresSaver`) in `backend/app/agent/graph.py`.
2. **DEV-024.2 [Nidhish Dalvi]:** Implement risk assessment logic in `plan_node` flagging tasks as `CRITICAL` when remaining life $< 2.0\text{ years}$.
3. **DEV-024.3 [Samar Shetye]:** Configure graph interrupt: `interrupt_before=["human_checkpoint"]` when task risk level is evaluated as `HIGH` or `CRITICAL`.
4. **DEV-024.4 [Tejas Rawool]:** Polish frontend `ApprovalPanel.tsx` modal with diff viewer showing original vs edited recommendation, operator ID input, and sign-off buttons.
...
### Developer Responsibilities (In Execution Order)
1. **Samar Shetye (Checkpoint State Architect):** Configures `AsyncPostgresSaver`, implements the deterministic graph interrupt, and builds the resume handler.
2. **Nidhish Dalvi (Risk Classifier Specialist):** Configures risk-scoring logic that flags hazardous corrosion rates and triggers mandatory human intervention.
3. **Tejas Rawool (Approval Interface Lead):** Builds the interactive `ApprovalPanel` modal, allowing operators to review, edit, or reject recommendations.
...
### Collaboration Flow
Samar Shetye and Nidhish Dalvi implement the state pause and risk triggers; Tejas Rawool builds the modal; Dakshit verifies state updates; Shreya secures digital signatures; Safa tests crash resilience.
...
### Review
Tejas Rawool reviews approval modal UX; Samar Shetye reviews checkpointer serialization; Shreya verifies tamper-evident approval logging.
...
3. **DEV-025.3 [Nidhish Dalvi]:** Format synthesized executive summary text and engineering recommendations for document inclusion.
...
5. **DEV-025.5 [Samar Shetye]:** Implement secure deliverable download endpoint `GET /api/tasks/{id}/download` streaming files with proper MIME types.
6. **DEV-025.6 [Tejas Rawool]:** Build deliverable download card in frontend `MessageBubble.tsx` showing file name, size, SHA-256 hash stamp, and direct download button.
...
2. **Nidhish Dalvi (Executive Summary Formatter):** Structures the AI recommendations and standards citations into publication-ready technical text.
...
4. **Samar Shetye (Download Stream Specialist):** Implements `GET /api/tasks/{id}/download` in FastAPI with verified file headers.
5. **Tejas Rawool (Download Card UI Lead):** Renders the document download card in the chat interface with file size and hash stamps.
...
### Collaboration Flow
Dakshit leads document styling; Nidhish Dalvi formats the content; Shreya adds cryptographic hashes; Samar Shetye builds the download route; Tejas Rawool renders the UI card; Safa performs visual QA.
...
### Review
Safa reviews document visual aesthetics; Samar Shetye reviews streaming endpoints; Shreya audits cryptographic hash matching.
...
### Task Sequence
1. **DEV-026.1 [Tejas Rawool]:** Connect central Zustand store (`useTaskStore.ts`) to live SSE stream, eliminating all remaining mock data dependencies.
2. **DEV-026.2 [Tejas Rawool]:** Polish the 3-column industrial layout (Sidebar templates, Center chat workspace, Right ContextPanel).
...
4. **DEV-026.4 [Nidhish Dalvi]:** Review chat feed typography and markdown rendering; ensure code blocks, tables, and mathematical formulas display with high contrast.
5. **DEV-026.5 [Samar Shetye]:** Verify that backend SSE stream feeds all necessary properties to the frontend store without requiring secondary REST polling calls.
...
### Developer Responsibilities (In Execution Order)
1. **Tejas Rawool (Frontend Consolidation Lead):** Binds the Zustand store to live backend SSE events and polishes the Black + Safety Orange theme (`#FF6A00`).
...
3. **Nidhish Dalvi (Typography & Content Auditor):** Validates markdown rendering for engineering formulas, code snippets, and table layouts.
4. **Samar Shetye (API Optimization Specialist):** Ensures SSE event payloads provide complete state snapshots, eliminating redundant client REST calls.
...
### Collaboration Flow
Tejas Rawool leads UI consolidation; Dakshit integrates charts; Nidhish Dalvi audits typography; Samar Shetye optimizes payloads; Shreya enforces client security; Safa validates responsiveness.
...
### Review
Tejas Rawool and Safa review visual design consistency; Shreya reviews DOM sanitization; Samar Shetye verifies WebSocket/SSE connection efficiency.
...
### Task Sequence
1. **DEV-027.1 [Tejas Rawool]:** Execute intake: upload scanned NDT report `HC_102_B_UT_Inspection_Report.pdf` and defect photo `corrosion_flange.png` in UI.
2. **DEV-027.2 [Dakshit]:** Verify PaddleOCR extracts wall-thickness table (Point P-01: $3.20\text{ mm}$ vs $6.02\text{ mm}$ nominal).
3. **DEV-027.3 [Nidhish Dalvi]:** Verify Adaptive Model Router selects `Qwen2.5-VL` and `Qwen2.5-14B` with explainable rationale displayed in UI.
4. **DEV-027.4 [Dakshit]:** Verify Hybrid RAG retrieves API 570 Section 7 Table 4 from Qdrant with citation card rendered.
5. **DEV-027.5 [Samar Shetye]:** Verify Docker sandbox executes corrosion rate calculation ($CR = 0.564\text{ mm/yr}$, $RL = 1.24\text{ yrs}$) with `--network none`.
6. **DEV-027.6 [Nidhish Dalvi]:** Verify Self-RAG critique passes (`ISREL = 0.95`, `ISSUP = 1.00`).
7. **DEV-027.7 [Tejas Rawool]:** Verify execution pauses at 4-Eye Approval Checkpoint; operator edits replacement window to 6 months and approves.
...
### Developer Responsibilities (In Execution Order)
1. **Tejas Rawool (Golden Path Operator):** Drives the UI workflow, uploads inspection documents, and validates node illumination on the timeline.
...
3. **Nidhish Dalvi (AI Reasoning Validator):** Validates router model assignment, prompt context formatting, and critique score thresholds.
4. **Samar Shetye (Backend & Sandbox Monitor):** Traces backend server logs, container execution in the sandbox, and state persistence.
...
3. **DEV-028.3 [Samar Shetye]:** Implement background async polling loop in FastAPI lifespan event, running socket audits every 2.0 seconds with minimal CPU footprint ($<1\%$).
4. **DEV-028.4 [Nidhish Dalvi & Dakshit]:** Test Ollama and Qdrant loopback sockets; ensure local inference sockets are recognized as authorized internal traffic.
...
7. **DEV-028.7 [Tejas Rawool]:** Connect `NetworkSentinelView.tsx` widget in UI to live `/api/network/status` endpoint, displaying green "AIR-GAP LOCKED" status and live packet counters.
...
2. **Samar Shetye (Background Poller Engineer):** Integrates the socket poller into FastAPI's background lifespan runner, maintaining low CPU usage.
3. **Nidhish Dalvi (LLM Loopback Auditor):** Verifies that local Ollama socket traffic on port 11434 is cleanly whitelisted.
...
6. **Tejas Rawool (Sentinel UI Lead):** Binds the live network telemetry data to the `NetworkSentinelView` widget in the frontend.

### Collaboration Flow
Shreya and Samar Shetye build the socket monitor; Nidhish Dalvi and Dakshit test service whitelisting; Safa executes breach tests; Tejas Rawool visualizes telemetry in the UI.
...
### Review
Samar Shetye reviews poller CPU utilization; Shreya verifies foreign socket interception; Tejas Rawool checks UI telemetry refresh rate.
...
3. **DEV-029.3 [Samar Shetye]:** Test database, FastAPI, and SSE operation under strict `iptables` drop policy; verify internal communication functions unimpeded.
4. **DEV-029.4 [Tejas Rawool]:** Test frontend operation while machine is completely offline; verify fonts, CSS, and icons load from local bundles with zero CDN dependencies.
...
6. **DEV-029.6 [Nidhish Dalvi]:** Verify Ollama and local LLMs execute offline without attempting HuggingFace or license server check-ins.
...
2. **Samar Shetye (Internal Network Verifier):** Verifies that container bridge networking and localhost HTTP/SSE calls operate seamlessly under firewall drop rules.
3. **Tejas Rawool (Offline Asset Auditor):** Verifies that all UI fonts, Tailwind styles, and icons render locally with zero external CDN requests.
...
5. **Nidhish Dalvi (Offline LLM Inference Auditor):** Verifies that Ollama runs inferences completely offline without external license check-ins.
...
### Collaboration Flow
Shreya writes and applies the firewall rules; Samar Shetye, Tejas Rawool, Dakshit, and Nidhish Dalvi verify subsystem independence; Safa conducts physical disconnect tests.
...
### Review
Shreya reviews `tcpdump` packet capture logs; Samar Shetye verifies localhost routing; Tejas Rawool confirms local asset loading.
...
### Task Sequence
1. **DEV-030.1 [Samar Shetye & Shreya]:** Create unified root `docker-compose.yml` defining all 5 services, volume mounts, health checks, and restart policies.
...
3. **DEV-030.3 [Samar Shetye]:** Optimize `backend/Dockerfile` with non-root user execution, multi-stage builds, and health check curl command.
4. **DEV-030.4 [Tejas Rawool]:** Optimize `frontend/Dockerfile` for multi-stage standalone Next.js production build (`node:20-alpine`), minimizing image size to $<180\text{MB}$.
...
6. **DEV-030.6 [Nidhish Dalvi]:** Verify Ollama container volume mounts; ensure pre-pulled model weights in `ollama_models` are recognized immediately without re-downloading.
...
### Developer Responsibilities (In Execution Order)
1. **Samar Shetye (Compose Infrastructure Lead):** Assembles the master `docker-compose.yml`, configuring service dependencies, health checks, and restart policies.
...
3. **Tejas Rawool (Frontend Containerization Lead):** Builds the multi-stage Alpine Dockerfile for Next.js, producing a lightweight, standalone image.
...
5. **Nidhish Dalvi (Model Storage Persistence Lead):** Configures Docker named volumes for Ollama, ensuring model weights remain cached across boots.
...
### Collaboration Flow
Samar Shetye and Shreya architect the compose stack; Tejas Rawool packages the frontend; Dakshit and Nidhish Dalvi configure persistent volumes; Safa tests cold boots.
...
### Review
Tejas Rawool reviews frontend container build; Samar Shetye reviews backend health checks; Shreya audits container network isolation.
...
### Task Sequence
1. **DEV-031.1 [Samar Shetye]:** Consolidate all unit, integration, and security tests into a single master test runner `scripts/run_all_tests.sh`.
...
4. **DEV-031.4 [Nidhish Dalvi]:** Test model router with ambiguous, multilingual, and nonsensical queries; verify fallback routing behaves deterministically.
5. **DEV-031.5 [Tejas Rawool]:** Run frontend test suite (`npm run test`); verify error boundary components catch unexpected rendering errors gracefully.
...
### Developer Responsibilities (In Execution Order)
1. **Samar Shetye (Master Test Automation Lead):** Builds `scripts/run_all_tests.sh`, consolidating backend unit tests, integration tests, and API checks.
...
4. **Nidhish Dalvi (Adversarial Prompt Fuzzer):** Submits ambiguous and adversarial queries to verify the router and policy engine fail safely.
5. **Tejas Rawool (Frontend Error Boundary Lead):** Tests React error boundaries, ensuring UI displays clean error cards rather than blank screens.
...
### Collaboration Flow
Samar Shetye organizes the master test runner; Shreya conducts security scans; Dakshit, Nidhish Dalvi, and Tejas Rawool test subsystem edge cases; Safa audits results.
...
### Review
Shreya reviews SAST vulnerability reports; Samar Shetye reviews test coverage reports; Tejas Rawool reviews error boundaries.
...
### Task Sequence
1. **DEV-032.1 [Samar Shetye]:** Execute simulated database crash: kill PostgreSQL container mid-transaction, restart container, and verify database auto-recovers with zero corrupted records.
2. **DEV-032.2 [Nidhish Dalvi]:** Execute simulated Ollama crash: terminate Ollama process during active inference, verify backend returns clean timeout event, and test auto-reconnection.
3. **DEV-032.3 [Nidhish Dalvi]:** Implement request queue throttling in `llm.py` preventing concurrent inferences from triggering CUDA OOM crashes.
4. **DEV-032.4 [Tejas Rawool]:** Test frontend reconnection resiliency: verify UI reconnects to SSE stream upon server restart without duplicate message rendering.
...
### Developer Responsibilities (In Execution Order)
1. **Samar Shetye (Database Crash & Recovery Lead):** Simulates database failures, testing connection pool re-establishment and transaction recovery.
2. **Nidhish Dalvi (LLM Crash & Throttling Lead):** Simulates model inference crashes, implementing request throttling to prevent CUDA memory panics.
3. **Tejas Rawool (Client Reconnect Specialist):** Implements exponential backoff on SSE client reconnects, maintaining conversation history.
...
### Collaboration Flow
Samar Shetye and Nidhish Dalvi inject backend crashes; Tejas Rawool and Dakshit ensure client and data stability; Shreya executes database restores; Safa logs RTO metrics.
...
### Review
Samar Shetye reviews database recovery integrity; Shreya verifies encrypted backup restoration; Tejas Rawool checks frontend reconnect states.
...
### Task Sequence
1. **DEV-033.1 [Samar Shetye]:** Create automated 1-Click Demo launcher script `scripts/start_demo.sh` that boots containers, pre-warms Ollama models, and opens the browser.
2. **DEV-033.2 [Nidhish Dalvi]:** Implement model pre-warming: send lightweight warm-up ping to load `Qwen-14B` and `Qwen-VL` weights into GPU VRAM before presentation.
3. **DEV-033.3 [Tejas Rawool]:** Lead the UI presentation walkthrough; rehearse screen narration and interactive feature highlighting; pace operator clicks.
...
### Developer Responsibilities (In Execution Order)
1. **Samar Shetye (Demo Automation Engineer):** Authors `scripts/start_demo.sh` ensuring all services boot and reach readiness with one command.
2. **Nidhish Dalvi (Model Pre-Warmer & Speaker):** Wires model pre-warming to eliminate first-token inference latency, and presents the Adaptive Router segment.
3. **Tejas Rawool (Lead UI Presenter):** Drives the live browser demonstration, pacing navigation and highlighting key industrial UI elements.
...
### Task Sequence
1. **DEV-034.1 [Samar Shetye]:** Enforce strict production code freeze on `main` branch; tag release `v1.0.0-Abhedya-Production`.
2. **DEV-034.2 [Samar Shetye & Shreya]:** Package complete offline air-gapped deployment bundle `scripts/package_airgap_bundle.sh` (Docker images, Ollama models, Qdrant vectors, SQL dump).
...
4. **DEV-034.4 [Dakshit & Nidhish Dalvi]:** Finalize multimodal, RAG, and model registry documentation across `docs/06-data/` and `docs/07-ai-ml/`.
5. **DEV-034.5 [Tejas Rawool]:** Review all frontend documentation; ensure screenshots and UI architecture diagrams match the final production application.
...
### Developer Responsibilities (In Execution Order)
1. **Samar Shetye (Release Manager):** Enforces branch freeze, cuts release tag `v1.0.0-Abhedya-Production`, and packages container tarballs.
...
4. **Nidhish Dalvi (Router & Model Documentation Lead):** Finalizes documentation for the Adaptive Model Router and Constitutional Policy guardrails.
5. **Tejas Rawool (Frontend Documentation Lead):** Updates screenshots, color token specs, and UI component hierarchy in the documentation hub.
...
### Collaboration Flow
Samar Shetye and Shreya package the offline distribution assets and produce the video; Dakshit, Nidhish Dalvi, and Tejas Rawool finalize technical docs; Safa audits all documentation links.
...
4. **DEV-035.4 [Cross-Assigned Injected Bug Challenges]:**
   - **Tejas Rawool:** Diagnoses and fixes a broken SQL migration script in backend.
   - **Dakshit:** Diagnoses and fixes an SSE connection drop in frontend.
   - **Nidhish Dalvi:** Diagnoses and fixes an `iptables` syntax error in security scripts.
   - **Samar Shetye:** Diagnoses and fixes a prompt injection vulnerability in policy engine.
...
### Developer Responsibilities (In Execution Order)
1. **Tejas Rawool (Cross-Domain Challenge: Database Migration):** Performs fresh clone setup; diagnoses and fixes an injected SQL migration bug; explains fix to Samar Shetye.
2. **Dakshit (Cross-Domain Challenge: Frontend SSE Streaming):** Performs fresh clone setup; diagnoses and fixes an injected SSE connection drop; explains fix to Tejas Rawool.
3. **Nidhish Dalvi (Cross-Domain Challenge: Firewall Security):** Performs fresh clone setup; diagnoses and fixes an injected `iptables` rule error; explains fix to Shreya.
4. **Samar Shetye (Cross-Domain Challenge: Constitutional Policy):** Performs fresh clone setup; diagnoses and fixes an injected prompt injection vulnerability; explains fix to Nidhish Dalvi.
...
6. **Safa (Cross-Domain Challenge: Docker Sandbox Execution):** Performs fresh clone setup; diagnoses and fixes an injected container cgroups memory exception; explains fix to Samar Shetye.
...
4. **DEV-036.4 [Samar Shetye & Safa]:** Archive all development logs, update repository documentation, and commit final `developer_plan.md`.
5. **DEV-036.5 [Tejas Rawool]:** Formal handover to team lead and final preparation for competition day.

### Developer Responsibilities (In Execution Order)
1. **Tejas Rawool (Level 2 & 6 Defense Lead):** Defends the 9-layer system blueprint, data trust perimeters, and the Next.js 16 SSE architecture; conducts the live UI demo.
...
3. **Nidhish Dalvi (Level 1 & 3 Defense Lead):** Defends refinery industrial context, SLM VRAM sizing, Adaptive Model Routing heuristics, and Constitutional Policy guardrails.
4. **Samar Shetye (Level 3 & 5 Defense Lead):** Defends LangGraph StateGraph orchestration, PostgreSQL relational schemas, and Docker Compose orchestration.
